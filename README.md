...
В актуальных клиентах Telegram основные действия выделяются цветом: синим — обновление и повтор, зелёным — запуск и скачивание, красным — окончательное подтверждение удаления. В `/new`, `/subs` и HTML-экранах админ-панели даты показываются в часовом поясе пользователя; текст внутри сообщения остаётся fallback для старых клиентов.
...
Для неполных сезонов, в том числе с одной доступной серией (`Серии: 1 из 10`), в этом же меню появляются дополнительные варианты:
...
- после завершения загрузки проверяет имена файлов сериала; если эпизоды явно не в Plex-формате, предлагает план переименования (русские арки, `Фильм N...часть M`, простая нумерация вроде `01.avi`) и не запускает ожидание Plex до решения пользователя; если уведомление пропустили, тот же экран можно открыть из карточки задачи или командой `/normalize`;
...
- в радаре и push по несматченным файлам имя файла ведёт в Plex Web на карточку metadata, если Plex уже отдал `ratingKey`; если файл удалось однозначно связать с загрузкой из поиска, рядом показывается ссылка на исходную раздачу.
...
Распознавание одиночной серии и создание подписки после поиска через Jackett/Rutracker покрыты в `tests/test_handlers.py` и `tests/test_subscription_picker_ui.py`.
