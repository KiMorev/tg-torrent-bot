...
После отправки ссылки `youtube.com/watch`, `youtu.be` или `youtube.com/shorts` бот показывает название, канал, длительность, доступные качества и кнопку скачивания. Очередь хранится в `youtube_downloads.json`; активная загрузка одна, чтобы не перегружать DS223. Прогресс показывается тем же форматом карточки, что и обычные загрузки, а после завершения бот присылает отдельное уведомление. Файл сохраняется в `/youtube_storage/<channel>/<title>/<title>.mp4` вместе с `poster.jpg`, `fanart.jpg` и `info.json`; `video_id` и дата остаются в `info.json`, а не в названии Plex-карточки.
