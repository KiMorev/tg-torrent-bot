...
Команда `/continue` помогает вернуться к неполному сезону, который уже есть в Plex. Бот собирает быстрый список из Plex и истории загрузок, показывает режимы `🙋 Моё` / `🌐 Всё`, листает кандидатов по 10 штук и открывает карточку сезона: сколько серий есть в Plex, какая прошлая тема известна и какой профиль скачивания был сохранён.
