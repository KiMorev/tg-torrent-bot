...
После отправки ссылки `youtube.com/watch`, `youtu.be` или `youtube.com/shorts` бот показывает название, канал, длительность, доступные качества и кнопку скачивания. Очередь хранится в `youtube_downloads.json`; активная загрузка одна, чтобы не перегружать DS223. Статусная карточка показывает checklist: очередь, подготовка, скачивание видео, при раздельном формате скачивание аудио, сборка MP4, обложка и metadata. После завершения карточка удаляется, а бот присылает отдельное уведомление. Файл сохраняется в `/youtube_storage/<channel>/<title>/<title>.mp4` вместе с `poster.jpg`, `fanart.jpg` и `info.json`; `video_id` и дата остаются в `info.json`, а не в названии Plex-карточки. В MP4 бот также прописывает `title`, `artist=<channel>`, `album=<channel>` и дату, чтобы Plex мог использовать канал как collection/category при включённых локальных metadata.
