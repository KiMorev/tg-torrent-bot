...
| `TMDB_API_TOKEN` | Включает точное определение количества серий в сезоне для `/seasons` и поиск целиком отсутствующих сезонов по Plex external ids. TVmaze без ключа дополнительно проверяет `imdb`/`tvdb` совпадения и отсекает конфликтующие totals. Нужен верхний TMDB `API Read Access Token`. |
...
Для полной точности `/seasons` нужен `TMDB_API_TOKEN`: Plex даёт external ids сериала, а TMDB по ним возвращает точное количество эпизодов в конкретном сезоне и список сезонов сериала для отсутствующих сезонов. Если у Plex есть `imdb`/`tvdb`, бот дополнительно сверяет сезон с TVmaze без отдельного ключа и скрывает Plex-only кандидат при конфликте нумерации сезонов. Без TMDB бот не считает Plex `leafCount` общим размером сезона; часть проверок может сработать по TVmaze, но сезон не показывается как уверенно неполный без внешнего total.
...
| `SERIES_CONTINUE_TOTALS_FILE` | JSON-кэш количества эпизодов сезона из TMDB/TVmaze для `/seasons`. |
...
| `/seasons` | разрешённые пользователи | Найти в Plex неполные и целиком отсутствующие сезоны, затем передать их в bulk-план скачивания. |
