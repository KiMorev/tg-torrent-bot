...
| `TMDB_API_TOKEN` | Включает точное определение количества серий в сезоне для `/continue` и поиск целиком отсутствующих сезонов по Plex external ids. TVmaze без ключа дополнительно проверяет `imdb`/`tvdb` совпадения и отсекает конфликтующие totals. Нужен верхний TMDB `API Read Access Token`. |
...
Для полной точности `/continue` нужен `TMDB_API_TOKEN`: Plex даёт external ids сериала, а TMDB по ним возвращает точное количество эпизодов в конкретном сезоне и список сезонов сериала для режима отсутствующих сезонов. Если у Plex есть `imdb`/`tvdb`, бот дополнительно сверяет сезон с TVmaze без отдельного ключа и скрывает Plex-only кандидат при конфликте нумерации сезонов. Без TMDB бот не считает Plex `leafCount` общим размером сезона; часть проверок может сработать по TVmaze, но сезон не показывается как уверенно неполный без внешнего total.
...
| `/continue` | разрешённые пользователи | Найти в Plex неполные сезоны, отдельно проверить целиком отсутствующие сезоны и передать их в bulk-план скачивания. |
...
| [`series_continue.py`](series_continue.py) | Чистые модели `/continue`: Plex identity, detector неполных/отсутствующих сезонов, completeness resolver и проверка той же темы. |
