...
- Пока трекеры отвечают, бот может показать короткий кинофакт; повторы отслеживаются отдельно для каждого чата, а тематические совпадения берутся из локальных alias-тегов. При включённом GPT бот может редко обновлять runtime-каталог фактов в фоне, но обычный поиск GPT для фактов не вызывает.
...
- каталог фактов ожидания: источник, процент показанного и последний GPT-refresh;
...
Голосовой поиск использует Whisper API (`whisper-1`). GPT используется точечно: разбор сложного поискового запроса, did-you-mean и объяснение пустой выдачи, проверка KP-матча, бейджи релизов, пояснения `/new`, подсказки спорных bulk-кандидатов, tie-break близких раздач в push `/new`, метаданные для ручных magnet/`.torrent` и редкое фоновое обновление каталога кинофактов ожидания. Это pay-per-use, без подписки; лимиты лучше выставить в кабинете OpenAI.
...
- `🤖 GPT / Voice` — голосовой поиск, GPT-улучшения и фоновое обновление каталога фактов ожидания.
