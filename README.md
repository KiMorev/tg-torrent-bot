...
После отправки ссылки `youtube.com/watch`, `youtu.be` или `youtube.com/shorts` бот показывает название, канал, длительность, доступные качества и кнопку скачивания. Очередь хранится в `youtube_downloads.json`; активная загрузка одна, чтобы не перегружать DS223. Статусная карточка показывает checklist: очередь, подготовка, скачивание видео, при раздельном формате скачивание аудио, сборка MP4, обложка и metadata. Временные сетевые сбои YouTube бот повторяет несколько раз и показывает retry в этой же карточке; если скачать всё равно не удалось, он присылает короткую причину без технических URL и чистит временные `.part`-файлы задачи. После завершения карточка удаляется, а бот присылает отдельное уведомление. Файл сохраняется в `/youtube_storage/<channel>/<title>/<title>.mp4` вместе с `poster.jpg`, `fanart.jpg` и `info.json`; `video_id` и дата остаются в `info.json`, а не в названии Plex-карточки. В MP4 бот также прописывает `title`, `artist=<channel>`, `album=<channel>` и дату, чтобы Plex мог использовать канал как collection/category при включённых локальных metadata. Когда Plex находит ролик, бот best-effort пробует назначить `poster.jpg` постером коллекции канала и закрепить этот постер, чтобы Plex не пересобирал автоколлаж.
