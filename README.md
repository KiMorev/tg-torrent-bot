...
Предпочитаемые переводы — мягкий приоритет: подходящие релизы поднимаются выше, а если такого перевода нет, бот показывает другие варианты с пояснением. В bulk-подборе перевод из `/settings` или запроса тоже работает как мягкий бонус к подбору сезонов; строгим ограничением он становится только после ручного выбора озвучки в профиле.
