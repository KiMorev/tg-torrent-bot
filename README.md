...
- Пока трекеры отвечают, бот может показать короткий кинофакт; повторы отслеживаются отдельно для каждого чата, а тематические совпадения берутся из локальных alias-тегов без GPT/API.
