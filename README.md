...
Для неполных сезонов бот показывает два типа кнопок:

- `⬇️ N` — действия со скачиванием:
  - `⬇️ Скачать сейчас`
  - `⬇️ Скачать сейчас + обновлять`
  - `📦 Дождаться полного сезона и скачать`
- `🔔 N` — действия только с уведомлениями:
  - `🔔 Уведомлять о новых сериях`
  - `🎯 Сообщить, когда сезон завершится`

Подписка хранит две независимые политики: когда уведомлять и когда скачивать. Поэтому бот может, например, молча ждать финала сезона и скачать его одним торрентом, либо уведомлять о каждой новой серии без автозагрузки.
...
```mermaid
flowchart TD
    result["Неполная раздача сериала"] --> download["⬇️ N"]
    result --> notify["🔔 N"]
    download --> now["Скачать сейчас"]
    download --> now_updates["Скачать сейчас + обновлять"]
    download --> wait_full["Скачать полный сезон после финала"]
    notify --> each["Уведомлять о новых сериях"]
    notify --> final["Сообщить о финале"]
```
...
Plex token можно взять в Plex Web: откройте любой фильм, DevTools → Network, найдите запрос к Plex и параметр `X-Plex-Token`.
...
| `/subs` | разрешённые пользователи | Активные подписки пользователя. |
