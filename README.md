...
| `/continue` | разрешённые пользователи | Найти в Plex неполные сезоны по истории загрузок и открыть карточку сезона для дальнейшего действия. |
...
| [`docs/series-catch-up-download.md`](docs/series-catch-up-download.md) | Проектный план и ограничения сценария `/continue`. |
