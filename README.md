...
- Поиск учитывает личные настройки по умолчанию из `/settings`: качество, Original, субтитры и предпочитаемые переводы.
...
Команда `/settings` открывает личные настройки поиска. Они применяются только к новым поискам и не меняются из результатов:
...
Предпочитаемые переводы — мягкий приоритет: подходящие релизы поднимаются выше, а в bulk-подборе озвучка предвыбирается только если она есть в эталонной раздаче. Явная фраза в запросе, например `в озвучке Кубик`, всегда сильнее настроек по умолчанию.
...
- умеет присылать push о новых подходящих фильмах подписчикам `/new`; если Кинопоиск отдал постер, push показывает постер главной новинки и обычный текстовый fallback при ошибке.
...
| `/settings` | разрешённые пользователи | Личные настройки поиска по умолчанию: качество, Original, субтитры и предпочитаемые переводы. |
