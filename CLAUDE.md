...
Hook `PostToolUse(Bash, git push*)` в `~/.claude/settings.json` watches CI runs в фоне. При failure меня будит через `asyncRewake` — не нужно помнить проверять самому.
...
- `movie_discovery: supplemented N releases from prev cache for failed specs: [(year, quality), …]` — сколько релизов перенесли из прошлого кэша вместо реального fetch'a. Без этого «Jackett для 2026 timed out, Rutracker 521» переписал бы топ-10 фрагментами успешных запросов (все 2025).
