...
# Approximate per-1M-tokens pricing for gpt-4o-mini (used for cost estimates
# in /admin). Hard-coded since OpenAI doesn't expose pricing programmatically.
_PRICE_INPUT_PER_1M = 0.150
_PRICE_OUTPUT_PER_1M = 0.600


def estimate_chat_cost_usd(input_tokens: int, output_tokens: int) -> float:
    """Estimate USD cost of a chat completion given token counts from the API."""
    return (
        (max(0, input_tokens) / 1_000_000) * _PRICE_INPUT_PER_1M
        + (max(0, output_tokens) / 1_000_000) * _PRICE_OUTPUT_PER_1M
    )
