...
- **Прод обновляется через GitHub/GHCR + Watchtower.** Штатный путь деплоя: commit/push в GitHub → GitHub Actions собирает и публикует Docker image в GHCR → Watchtower на NAS мониторит новый digest, подтягивает образ и пересоздаёт контейнер. Поэтому после обычного push локальный `docker compose up -d --build` не нужен; он нужен только для локальной сборки из checkout.
- **GitHub CLI для Codex.** `gh` установлен и авторизован под `KiMorev` в Windows keyring. Обычный sandbox Codex запускается как `codexsandboxoffline` и не видит keyring напрямую, поэтому команды `gh` для CI/PR/logs нужно запускать с escalation вне sandbox. Проверено: `gh auth status` и `gh run list --repo KiMorev/tg-torrent-bot --limit 5` работают вне sandbox.
- **Сообщать про ручное применение инфраструктурных изменений.** Если в задаче менялись `compose.yaml`, `Dockerfile`, `.env.example`, переменные окружения, зависимости, установщик или способ запуска, в финальном ответе обязательно явно сказать, нужно ли пользователю пересоздать/пересобрать контейнер, и дать точную команду. Помнить: `docker compose restart` не применяет новые значения `.env`/`compose.yaml`; для них нужен `docker compose up -d`, а для локальной сборки образа — `docker compose up -d --build`.
...
- `docs/bot-structure.md` — быстрый навигатор по модулям, handlers, callback namespaces, state-файлам и тестам.
- При любом изменении в проекте проверять, нужно ли обновить `docs/bot-structure.md`.
- Обновлять обязательно в той же задаче, если менялись команды, callback-data, пользовательские флоу, фоновые циклы, JSON-состояние, переменные окружения, интеграции, установщик или ответственность модулей.
- Если изменение локальное и не меняет карту маршрутов, отдельная правка `docs/bot-structure.md` не нужна.
...
- [ ] `docs/bot-structure.md` проверен/обновлён (если менялась структура бота)
...
### Докачать серии и сезоны (`/seasons`)

- `series_continue: metadata totals show=... rating_key=... cached_tmdb=N cached_tvmaze=N live_tmdb=N live_tvmaze=N selected=N conflicts=N` — сбор стартовавших сезонов для missing-проверки. Если `selected` меньше ожидаемого количества сезонов, смотреть `cached_*` vs `live_*`: старый/частичный кэш может быть отброшен TTL или `__version__`, будущие сезоны с нулём вышедших серий не попадают в список, а `conflicts>0` означает, что сезон не скрыт, но количество серий спорит между TMDB и TVMaze.
...
### Jackett Guardian (`_jackett_warmup_loop` / `_jackett_guardian_loop`)

- `jackett_guard: loop started` — запущен targeted retry для проблемных Jackett-индексеров.
- `jackett_guard: targeted retry indexers=...` — Guardian проверяет конкретные индексеры, у которых наступил `next_retry`.
- `jackett_guard: retry skipped before search reason=busy` — Jackett занят пользовательским поиском/скачиванием, targeted retry пропущен без влияния на пользователя.
- `jackett_guard: warmup events=indexer:state,...` — warmup/guardian изменил состояние индексера (`recovered`, `degraded`, `quarantined`, `manual_required`).
...
- `movie_discovery: supplemented N releases from prev cache (failed_indexers=... guard_unready=... failed_specs=...)` — сколько релизов перенесли из прошлого кэша. **`failed_indexers`** — Jackett-индексеры с Status=1 или Results=0+Error в текущем ответе. **`guard_unready`** — индексеры, которые Jackett Guardian ещё считает неподтверждёнными после прошлой ошибки. **`failed_specs`** — query-уровень, когда вообще ни один источник не вернул контент. Подтягиваем prev releases по `tracker in failed_indexers/guard_unready` (точечно) или по (year, quality) для full-fail-specs.
- `movie_discovery: Jackett guard marks indexers unready enabled=... disabled=... manual=...` — Guardian добавил память о проблемных индексерах к текущему refresh. `enabled` влияет на `/new` и ready/recovery, `disabled` info-only.
- `movie_discovery: degraded cache rejected, keeping previous cache prev_cards=X new_cards=Y ...` — новый refresh деградировал и построил подозрительно короткий список; хороший прошлый кэш сохранён, а failure-сигналы записаны для retry/backoff.
...
- `movie_discovery: notify skipped — degraded refresh` — push подавлен, потому что текущий refresh имеет gating-деградацию (`last_failed_specs` или enabled-indexer failures). Пользователь не получает уведомление по неполному/восполненному снимку.
