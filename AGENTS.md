...
- **Сначала обсуждаем изменения на берегу.** Перед нетривиальными правками Codex должен кратко сформулировать, что именно будет менять и где проходит граница задачи.
- **Commit/push только после явного подтверждения.** Даже если код и тесты готовы, Codex не коммитит и не пушит без прямой команды пользователя.
- **Длинные задачи ведём через roadmap/checklist.** Для серийного поиска и подписок текущий живой план: `docs/series-search-subscriptions-roadmap.md`.
