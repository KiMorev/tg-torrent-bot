...
- `series_continue: metadata totals show=... rating_key=... cached_tmdb=N cached_tvmaze=N live_tmdb=N live_tvmaze=N selected=N conflicts=N` — сбор каталога сезонов для missing-проверки. Если `selected` меньше ожидаемого количества сезонов, смотреть `cached_*` vs `live_*`: старый/частичный кэш может быть отброшен TTL, а `conflicts>0` означает, что сезон не скрыт, но количество серий спорит между TMDB и TVMaze.
