...
### ✅ 1.3b UI (presets + advanced)
**Статус:** ГОТОВО.

Сделано:
- Кнопка `🔔 N` в результатах поиска вместо пары `📺 Серии` / `🎯 Сезон`
- Тап → preset-picker (Style D): 4 пресета + ⚙️ Advanced + ⬅️ К результатам
- Hint-line над клавиатурой объясняет «push» / «качать»
- 5 callback handlers: `sub_pick`, `sub_preset:CODE`, `sub_advanced`,
  `sub_set_notify`, `sub_set_download`, `sub_back_results`
- Advanced — двухшаговое меню (notify → download), повторный тап на пресеты
  через «⬅️ Назад»
- `_download_and_add` принимает `notify_policy`/`download_policy` поверх
  legacy `notify_mode` — все 3 фоновых loop'а уже это используют
- 12 новых тестов в `test_subscription_picker_ui.py`
