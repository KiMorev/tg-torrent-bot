...
class NotificationDeduplicationTests(unittest.TestCase):
    ...
    def test_non_plex_episode_names_offer_rename_instead_of_polling(self) -> None:
        ...
        self.assertIn("Нашёл сезон: 1", text)
        labels = [
            button.text
    ...
    def test_unsafe_arc_episode_names_block_polling(self) -> None:
        ...
        show_dir.mkdir(parents=True)
        for name in [
            "1. Case (1 сер.) - hdtv1080p.mkv",
            "1. Case (3 сер.) - hdtv1080p.mkv",
        ]:
            (show_dir / name).write_bytes(b"")
        created_coroutines = []
    ...
    def test_complete_error_status_notifies_as_done_and_starts_plex_polling(self) -> None:
        ...
        with (
            ...
            patch.object(bot, "_PLEX_POLLING_TASKS", {}),
            patch.object(bot, "_plex_poll_is_done", return_value=False),
            patch.object(bot, "_get_task_meta", return_value={"kind": "movie", "title": "Movie"}),
            patch.object(bot, "_plex_poll_after_finish", AsyncMock()) as poll,
            patch.object(bot.asyncio, "create_task", side_effect=fake_create_task),
        ):
            asyncio.run(_run_task_notifications_once(mock_app))
    ...
    def test_complete_error_then_finished_does_not_duplicate_notification(self) -> None:
        ...
        finished_task = {**error_task, "status": "finished"}
        mock_app = MagicMock()
        mock_app.bot.send_message = AsyncMock()

        mock_ds = MagicMock()
        mock_ds.list_tasks.side_effect = [[error_task], [finished_task]]

        with (
            patch.object(bot, "ds_client", mock_ds),
            patch.object(bot, "state_store", self._store),
            patch.object(bot, "TASK_NOTIFICATIONS_ENABLED", True),
            patch.object(bot, "TASK_NOTIFICATION_STATUSES", {"finished", "error"}),
            patch.object(bot, "TASK_NOTIFY_EXTERNAL_TASKS", True),
            patch.object(bot, "ALLOWED_CHAT_IDS", {999}),
            patch.object(bot, "PLEX_ENABLED", False),
        ):
...
class PendingDownloadsLoopTests(unittest.TestCase):
    ...
    def test_success_without_task_id_warns_and_does_not_track(self) -> None:
        ...
        mock_app.bot.send_message.assert_awaited_once()
        sent_text = mock_app.bot.send_message.await_args.kwargs.get("text", "")
        self.assertIn("бот пока не видит созданную задачу", sent_text)
        self.assertIn("через минуту откройте список загрузок", sent_text)
...
class SubscriptionCheckTests(unittest.TestCase):
    ...
    def _season_candidate(self, *, present_count: int = 4):
        ...
        return SeriesCatchUpCandidate(
            identity=PlexSeriesIdentity(
                plex_rating_key="show-100",
                plex_guid="plex://show/100",
                title="The Rookie",
            ...
            present_episode_numbers=tuple(range(1, present_count + 1)),
            known_total=8,
            quality="1080p",
            source="plex",
        )

    def _season_subscription(self, candidate):
        return {
            ...
            "season": 8,
            "season_key": bot._series_continue_candidate_key(candidate),
            "present_count": candidate.present_count,
            "present_episode_numbers": list(candidate.present_episode_numbers),
            "known_total": candidate.known_total,
            "last_episode_end": candidate.present_count,
            "total_episodes": candidate.known_total,
            "quality": "1080p",
    ...
    def test_plex_season_subscription_auto_downloads_matching_missing_episodes(self) -> None:
        ...
        mock_rt.search.return_value = [
            self._rt_result("999", "The Rookie S8E1-6 of 8 WEB-DL 1080p"),
        ]
        mock_app = MagicMock()
        mock_app.bot.send_message = AsyncMock()
        ...
        with (
            ...
            patch.object(
                ...
                AsyncMock(return_value=({bot._series_continue_candidate_key(candidate): candidate}, set())),
            ),
            ...
            patch.object(bot, "_schedule_task_notification_fast_wake") as fast_wake,
        ):
    ...
    def test_plex_season_subscription_complete_in_plex_removes_subscription(self) -> None:
        ...
        with (
            ...
            patch.object(
                bot,
                "_plex_season_subscription_candidates_for_chat",
                AsyncMock(return_value=({}, {bot._series_continue_candidate_key(candidate)})),
            ),
        ):
            asyncio.run(bot._check_plex_season_subscriptions(mock_app))

        self.assertEqual(self._store.load_topic_subscriptions(), {})
        mock_rt.search.assert_not_called()
