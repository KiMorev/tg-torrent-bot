...
| Сезоны Plex | `/seasons` -> `series_continue_command` -> `_series_continue_build_state(include_missing=True)` собирает Plex-сериалы с сезонами, `download_history.jsonl`, строгие totals из TMDB+TVmaze для уже имеющихся сезонов и список целиком отсутствующих сезонов; для missing-сезонов факт существования сезона отделён от точного episode count: конфликты TMDB/TVmaze не скрывают сезон, а помечаются в тексте как спорное число серий; complete-снимки totals кэшируются в `series_continue_totals.json` с `__fetched_at__` и TTL 7 дней; точные Rutracker-подписки текущего пользователя по `topic_id` подсвечиваются из `topic_subscriptions.json`; первый экран всегда показывает неполные сезоны (`Моё` / `Всё` / скрытые), а `cont:list:missing:0` открывает список целиком отсутствующих сезонов; `cont:hide:*` / `cont:unhide:*` сохраняют персональное скрытие в `series_continue_hidden.json`; `cont:update_topic:*` проверяет текущий title той же Rutracker-темы, не создаёт дубль при активной задаче, добавляет обновлённый torrent и при неполном сезоне сохраняет подписку; если темы нет или она не обновилась, `cont:search_alt:*` показывает похожие Rutracker-кандидаты как обновлённые раздачи, а `cont:subscribe_topic:*` создаёт подписку без скачивания для известной темы; `cont:missing_refresh` обновляет missing-список через `_series_continue_build_missing_state`, а `cont:missing_bulk:*` передаёт выбранные Sxx в существующий bulk-план. |
...
| `tmdb.py` | TMDB API для точного количества эпизодов в сезоне и карты сезонов сериала `/seasons` по Plex external ids. |
| `tvmaze.py` | TVmaze API для high-confidence проверки totals и карты сезонов `/seasons` по Plex `imdb`/`tvdb` ids. |
...
| `series_continue_totals.json` | Кэш количества эпизодов сезона и карты сезонов из TMDB/TVmaze для `/seasons`, keyed by Plex/external id; complete-снимки содержат `__complete__` и `__fetched_at__`, старые снимки обновляются перед missing-проверкой. |
