...
| `cont:*` | `bot.py` | `series_continue_callback`: список `/continue`, переключение `Моё` / `Всё` / скрытые, refresh, карточка сезона, `cont:hide:*`, `cont:unhide:*`, `cont:update_topic:*`, `cont:subscribe_topic:*`, `cont:search_alt:*`, `cont:alt_dl:*` |
...
| `tmdb.py` | TMDB API для точного количества эпизодов в сезоне `/continue` по Plex external ids. |
| `tvmaze.py` | TVmaze API для high-confidence проверки totals `/continue` по Plex `imdb`/`tvdb` ids. |
