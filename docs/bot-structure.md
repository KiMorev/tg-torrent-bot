...
| `sub:*` | `bot.py`, частично `keyboards.py` | `sub_callback`: список/отписка/настройка подписок; `sub:jackett_view:*` маршрутизируется в `ConversationHandler` раньше общего `sub_callback` и открывает `search_jackett_check_entry` |
