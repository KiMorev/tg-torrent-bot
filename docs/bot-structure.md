...
| `cont:*` | `bot.py` | `series_continue_callback`: список `/continue`, переключение `Моё` / `Всё`, refresh, карточка сезона и `cont:update_topic:*` для докачивания из той же Rutracker-темы |
