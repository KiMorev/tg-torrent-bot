...
| `bot.py::setup_bot_commands` | Обновляет меню команд, чистит temp-dir, запускает фоновые задачи; при `YOUTUBE_DOWNLOADS_ENABLED=true` запускает YouTube worker и автообновление YouTube status cards. |
