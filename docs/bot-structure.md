...
| `task:*` | `keyboards.py`, локально в `bot.py` для `task:norm_*` | `task_callback`: задачи Download Station, YouTube retry/delete (`retry_youtube`, `delete_youtube`, `delete_youtube_all`), закрытие сообщений и post-download переименование сериалов (`norm_plan`, `norm_apply`, `norm_pick`, `norm_season`, `norm_skip`) |
