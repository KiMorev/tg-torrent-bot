...
| `/status` и список задач | `status` / `task_callback` -> `task_views.py` + `keyboards.py`; список объединяет Download Station tasks и YouTube jobs из `youtube_downloads.json`; admin-view берёт владельцев DS из `task_owners.json`, YouTube-владельцев из job `chat_id`, подписи из `approved_users.json`. |
