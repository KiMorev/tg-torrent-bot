...
| YouTube-download | `bot.py`: `youtube_link_entry`, `youtube_callback`, `_youtube_worker_once`, `_youtube_plex_poll_after_finish`; `youtube_downloads.py`; `plex.py`; `diagnostics.py` | `tests/test_youtube_downloads.py`, `tests/test_handlers.py`, `tests/test_plex.py`, `tests/test_diagnostics.py`, `tests/test_keyboards.py` |
| Нормализация имён файлов сериалов | `bot.py`: `normalize_command`, `_normalization_candidates_screen`, `_normalization_entry_screen`, `_inspect_completed_task_normalization`, `_handle_normalization_callback`; `filename_normalizer.py`; `storage.py` | `tests/test_filename_normalizer.py`, `tests/test_background.py`, `tests/test_handlers.py`, `tests/test_keyboards.py` |
| История загрузок | `bot.py`: `_record_download_history`, `_record_download_added_history`, `_task_with_history_file_snapshot`, `_record_task_notification_history`, `_plex_poll_after_finish`; `download_station.py`: `get_task_info`; `state_store.py`: `download_history.jsonl`; подробности в `docs/download-history.md` | `tests/test_state_store.py`, `tests/test_handlers.py`, `tests/test_background.py`, `tests/test_download_station_locking.py` |
...
| Админ-панель и диагностика | `bot.py`: `admin_command`, `admin_callback`, `admin_broadcast_*`; `diagnostics.py`; `keyboards.py`; `storage.py` | `tests/test_handlers.py`, `tests/test_diagnostics.py`, `tests/test_storage.py`, `tests/test_keyboards.py` |
| Фоновые циклы | `bot.py`: `setup_bot_commands`, `_tracker_background_loop`, `_task_maintenance_loop`, `_schedule_task_notification_fast_wake`, `_subscription_check_loop`, `_movie_discovery_loop`, `_jackett_warmup_loop`, `_jackett_guardian_loop`, `_plex_cache_loop`; профильная логика в отдельных модулях | `tests/test_background.py`, профильные тесты блока |
...
| `bot.py::setup_bot_commands` / `shutdown_background_services` | Обновляет меню команд, чистит temp-dir, запускает фоновые задачи через managed task registry и гасит их на `post_stop`; при `YOUTUBE_DOWNLOADS_ENABLED=true` запускает YouTube worker и автообновление YouTube status cards. |
...
| План недостающих сезонов | `search_download_pick` -> `search_series_bulk_plan` показывает профиль; если voice hint из запроса или `/settings` есть в эталонной раздаче, он сохраняется как мягкий `preferred_voices` бонус, а не как hard-фильтр; `Original`/субтитры из `/settings` не становятся hard-требованиями, пока пользователь не выбрал их явно в текущем поиске или bulk-профиле; качество применяется посезонно: при наличии выбранного качества другие качества не авто-выбираются, а если выбранного качества по сезону нет, лучший fallback может стать `good` с предупреждением в плане -> `search_series_bulk_profile_callback` меняет профиль: главный экран, раскрываемый блок озвучки, ручной выбор 1-2 студий и `⚙️ Остальные настройки` -> `search_series_bulk_build_plan` запускает wide/targeted tracker search неблокирующим handler'ом; при fetch-limit широкий поиск расширяет targeted-pass до всех нужных сезонов; ожидательный экран использует search animation, обновляет этапы сборки и при долгой работе добавляет мягкий статус; `ConversationHandler.WAITING` принимает `srch:cancel`, выставляет cancel-token, сборка останавливается между сетевыми этапами; далее `series_bulk_planner.py`; GPT может добавить `gpt_hint` к спорным кандидатам, но не меняет статус и не запускает скачивание; если все сезоны уже есть в Plex или уже качаются, бот показывает финальный экран и не создаёт job; иначе создаётся `series_bulk_jobs.json` job -> `search_series_bulk_confirm` -> `search_series_bulk_run` для выбранных сезонов; если после batch остаются спорные/ненайденные сезоны, итоговый экран остаётся в search-flow и ведёт к `search_series_bulk_review` или обратно к плану; временные ошибки добавления уходят в `pending_downloads.json` с `series_bulk`-ссылкой и фоновый retry обновляет job; скрытый `/bulk` -> `series_bulk_command` -> `search_series_bulk_open` восстанавливает только actionable job после рестарта, а `cancelled`/`replaced` планы не показываются; `search_series_bulk_pack_list` -> `search_series_bulk_pack_confirm` -> `search_series_bulk_pack_run` вручную добавляет выбранный pack и помечает покрытые сезоны в job; `search_series_bulk_rebuild` помечает прежний job как `replaced` и возвращает готовый план к профилю для новой сборки; `search_series_bulk_review` разбирает `missing`/`needs_decision`/`partial` и постоянные ошибки добавления; выбор спорного кандидата только помечает сезон выбранным в плане и пишет это в job, а скачивание идёт через общий `search_series_bulk_run`; `search_series_bulk_soft_search` мягко добирает кандидатов для текущего сезона, `search_series_bulk_retry` повторяет failed-сезон, результат пишется в job. |
| Докачать серии и сезоны | `/seasons` -> `series_continue_command` -> `_series_continue_build_state(include_missing=True)` собирает Plex-сериалы с сезонами, episode indexes из Plex для имеющихся эпизодов, `download_history.jsonl`, строгие totals уже вышедших эпизодов из TMDB+TVmaze для имеющихся сезонов и список целиком отсутствующих стартовавших сезонов; будущие/ещё не стартовавшие сезоны из каталогов не считаются отсутствующими в Plex; для missing-сезонов факт существования сезона отделён от точного episode count: конфликты TMDB/TVmaze не скрывают сезон, а помечаются в тексте как спорное число серий, а временно недоступный каталог помечается рядом с источником, который подтвердил сезон; complete-снимки totals кэшируются в `series_continue_totals.json` с `__fetched_at__`, `__version__` и TTL 7 дней; подписки текущего пользователя на ту же Rutracker-тему подсвечиваются из `topic_subscriptions.json`, включая Jackett-подписки с Rutracker `topic_url`, а подписки `type=plex_season` подсвечиваются по season-key; первый экран всегда показывает неполные сезоны (`Моё` / `Всё` / скрытые), а `cont:list:missing:0` открывает список целиком отсутствующих сезонов; `cont:hide:*` / `cont:unhide:*` сохраняют персональное скрытие в `series_continue_hidden.json`; `cont:subscribe_season:*` сохраняет подписку на неполный Plex-сезон без привязки к topic_id; `cont:missing_subscribe:*` сохраняет такую же `type=plex_season` подписку для целиком отсутствующего сезона; `cont:update_topic:*` проверяет текущий title той же Rutracker-темы, не создаёт дубль при активной задаче, добавляет обновлённый torrent и при неполном сезоне сохраняет подписку; если темы нет или она не обновилась, `cont:search_alt:*` показывает похожие Rutracker-кандидаты как обновлённые раздачи, `cont:alt_dl:*` скачивает выбранный вариант и при partial сохраняет подписку, `cont:alt_sub:*` сохраняет подписку на выбранную тему без скачивания, а `cont:subscribe_topic:*` создаёт подписку без скачивания для известной темы; `cont:missing_refresh` обновляет missing-список через `_series_continue_build_missing_state`, а `cont:missing_bulk:*` передаёт выбранные Sxx в существующий bulk-план и при неизвестном качестве сезона использует личный/default quality, обычно `1080p`, вместо `any`. |
| Предохранитель похожих раздач `/seasons` | `cont:alt_dl:*` перед скачиванием сравнивает выбранную похожую раздачу с Plex episode indexes/историей; если недостающих для Plex серий нет, Download Station не вызывается, а для неполной раздачи сохраняется подписка на выбранную тему. |
...
| Подписка на сериал | `search_subscribe_pick` -> `search_subscribe_preset` или advanced callbacks -> запись в `topic_subscriptions.json`; `/seasons` -> `cont:subscribe_season:*` пишет `type=plex_season` для неполного Plex-сезона; `/subs` -> `sub:settings:*` меняет `notify_policy`/`download_policy`. |
| Проверка подписок | `_subscription_check_loop` -> `_check_subscriptions` для Rutracker-тем, `_check_plex_season_subscriptions` для подписок на Plex-сезоны и `_check_jackett_subscriptions` для Jackett. |
| `/new` | `movie_new_command` -> чтение cache/settings -> `movie_new_*` callbacks; refresh делает `_refresh_movie_discovery_cache`; при деградации источников и Jackett Guardian unready-indexers сохраняет прежний хороший кэш и не отправляет push; перед показом и push карточки фильтруются через подтверждение прошлым top-10; карточки показывают KP-метаданные включая страну; push подписчикам делает `_run_movie_discovery_notifications`, отправляет до 3 KP-обогащённых карточек не из Plex, в тихие часы кладёт компактный snapshot в `movie_notification_pending` и доставляет его первым в следующий разрешённый интервал, сохраняет `movie_notification_snapshots`, даёт `new:open:<push_id>` / `new:dl:*` / `new:bulk:*` для открытия и скачивания из snapshot, при изменившемся текущем `/new` подсвечивает исторический snapshot и даёт кнопку свежего списка, при близких score может вызвать GPT tie-break для выбора release, может отправить постер топовой карточки через `send_photo` с fallback на текст и отключает подписку после лимита permanent-ошибок доставки. |
| YouTube-ссылка | `text_message_entry` до обычного поиска проверяет `find_youtube_url`; если `YOUTUBE_DOWNLOADS_ENABLED=false`, отвечает сообщением с кнопкой открытия YouTube и не запускает torrent-search; если включено, `youtube_link_entry` получает metadata через `yt-dlp`, по `video_id` не даёт создать дубль активной/готовой YouTube-задачи и для уже найденного Plex-ролика даёт кнопку `▶️ Смотреть в Plex`; orphan-папки без готового `.mp4` внутри YouTube-root чистятся перед новым preview; если дубля нет, показывает качества от `YOUTUBE_MIN_HEIGHT` до `YOUTUBE_MAX_HEIGHT`, `youtube_callback` создаёт `youtube_downloads.json` job и пишет `youtube_download_queued`; `_youtube_worker_loop` держит одну активную загрузку, скачивает без транскодирования в `/youtube_storage/<channel>/<title>/<title>.mp4`, обновляет YouTube checklist-карточку через `_youtube_pipeline_card_text` с позицией в очереди и кнопкой к списку загрузок, повторяет временные сетевые сбои и чистит временные файлы при окончательном failure, пишет history events, через `YOUTUBE_AUDIO_LANGUAGE` проставляет язык аудиодорожки copy/remux, создаёт `poster.jpg`/`fanart.jpg` для ролика и `channel-poster.jpg|png` в папке канала; failed YouTube-задачу можно вернуть в очередь через `task:retry_youtube:*`; `channel-poster.jpg` собирается как вертикальный 2:3 постер из аватара с контрастной подложкой, старый квадратный jpg может быть перестроен при следующей загрузке канала; при включённом `YOUTUBE_PLEX_REFRESH_AFTER_DOWNLOAD` вызывает refresh секции и при включённом `YOUTUBE_PLEX_POLL_AFTER_DOWNLOAD` запускает lookup отдельной Plex-библиотеки `YouTube`; после найденного ролика best-effort загружает `channel-poster.*` как постер Plex-коллекции канала и лочит `thumb`, чтобы Plex не заменял его автоколлажем. |
| Прогрев Jackett | `_jackett_warmup_loop` / `_jackett_guardian_loop` -> `_run_jackett_warmup_once` -> `JackettClient.warmup`; индексеры прогреваются ротационными пачками, проблемные индексеры проверяются точечно по backoff, per-indexer состояние хранит `jackett_guard.py`, общий статус виден в диагностике. |
| Уведомление о завершении | `_task_maintenance_loop` -> `_run_task_notifications_once` -> Telegram push; обычные получатели берутся из владельца `task_owners.json` плюс активные viewers карточки задачи, при этом просмотр карточки не перезаписывает владельца; для уведомления о нормализации активные viewers не считаются получателями, чтобы админ не получал remediation-push только из-за просмотра карточки; при Plex включён сначала проверяет реальные файлы в `/storage`: если `filename_normalizer.py` строит безопасный план, нужен выбор сезона или имена похожи на эпизоды без надёжного `SxxEyy`, показывает `task:norm_*` кнопки и не стартует `_plex_poll_after_finish` до решения пользователя; иначе запускает обычный Plex polling. BT-задача `error` без конкретного `error_detail` (`unknown` считается неконкретным) и с прогрессом >=99.9% считается мягко завершённой для уведомлений/Plex polling; перед записью завершения бот точечно получает `detail,file` задачи, а итоговое событие сохраняет destination и относительные видеофайлы в `download_history.jsonl`. |
| `/status` и список задач | `status` / `task_callback` -> `task_views.py` + `keyboards.py`; список объединяет Download Station tasks и YouTube jobs из `youtube_downloads.json`, показывает источник задачи, для YouTube не выводит технические `video_id/job_id`, для DS использует обычную автоочистку/массовое удаление, а для завершённых BT-сериалов с безопасным планом переименования карточка показывает `task:norm_open:*`; для завершённых/ошибочных YouTube jobs даёт отдельное удаление ролика или всех YouTube-роликов с безопасной очисткой файлов из YouTube-папки и best-effort refresh Plex-библиотеки `YouTube`; если refresh после удаления не стартовал по временной причине, `youtube_plex_refresh_pending.json` даёт фоновый retry, `refresh_false` ограничен короткой серией, `section_not_found` повторяется один раз; admin-view берёт владельцев DS из `task_owners.json`, YouTube-владельцев из job `chat_id`, подписи из `approved_users.json`. |
| `/admin` | `admin_command` / `admin_callback` / `admin_broadcast_*` -> короткая диагностика и drill-down `admin:diag_*`, настройки `/new`, пользователи, подписки, рассылка всем пользователям с доступом, сброс счётчиков. |
...
| `task:*` | `keyboards.py`, локально в `bot.py` для `task:norm_*` | `task_callback`: задачи Download Station, YouTube retry/delete (`retry_youtube`, `delete_youtube`, `delete_youtube_all`), закрытие сообщений и post-download переименование сериалов (`norm_open`, `norm_plan`, `norm_apply`, `norm_pick`, `norm_season`, `norm_skip`) |
| `admin:*` | `keyboards.py`, admin-блок `bot.py` | `admin_callback`: панель, `admin:diagnostics`, `admin:diagnostics_back`, подробные `admin:diag_downloads` / `admin:diag_jackett` / `admin:diag_trackers` / `admin:diag_plex` / `admin:diag_youtube` / `admin:diag_ai`, refresh подробностей `admin:diag_refresh:*`; `admin_broadcast_*` conversation: `admin:broadcast`, `admin:broadcast_tpl:*`, `admin:broadcast_send`, `admin:broadcast_edit` |
| `access:*` | `keyboards.py` | `access_callback`: approve/deny заявок, `/users` refresh, подтверждение и удаление доступа |
| `sub:*` | `bot.py`, частично `keyboards.py` | `sub_callback`: список/отписка/настройка подписок; `sub:new_unsub` из фото-push `/new` сначала создаёт новое текстовое сообщение и удаляет старый push; `sub:jackett_view:*` маршрутизируется в `ConversationHandler` раньше общего `sub_callback` и открывает `search_jackett_check_entry` |
| `new:*` | `bot.py`, `keyboards.py` | `movie_new_*` callbacks; `new:open:<push_id>` открывает snapshot уведомления, старый `new:open` открывает текущий `/new`; `new:dl:*` входит в search conversation, а `new:dl:*` / `new:bulk:*` из фото-уведомления сначала создают новое текстовое сообщение и удаляют старый push; `new:bulk_ok:*` / `new:push_back:*` обрабатываются глобально |
| `yt:*` | `keyboards.py` | `youtube_callback`: подтверждение качества и постановка YouTube job в очередь; `yt:dl:<token>:<height>` |
| `cont:*` | `bot.py` | `series_continue_callback`: список `/seasons`, переключение `Моё` / `Всё` / скрытые, refresh, карточка сезона, `cont:hide:*`, `cont:unhide:*`, `cont:update_topic:*`, `cont:subscribe_topic:*`, `cont:subscribe_season:*`, `cont:search_alt:*`, `cont:alt_dl:*`, `cont:alt_sub:*`; missing-list: `cont:list:missing:*`, `cont:missing_refresh`, `cont:missing_open:*`, `cont:missing_hide:*`, `cont:missing_unhide:*`, `cont:missing_bulk:*`, `cont:missing_subscribe:*`. |
...
| `jackett_guard.py` | Чистая state-machine готовности Jackett-индексеров: `ok`/`warming`/`degraded`/`quarantined`/`manual_required`, backoff, due retry и unready summary для `/new`. |
...
| `series_continue.py` | Чистые модели `/seasons`: Plex identity, detector неполных/отсутствующих сезонов по episode indexes, completeness resolver и проверка той же темы. |
...
| `youtube_downloads.py` | YouTube URL parsing, metadata через `yt-dlp`, выбор MP4/H.264/AAC формата без транскодирования, retry временных download-сбоев, cleanup частичных файлов, план путей, sidecar-файлы, collection-friendly MP4 tags и audio-language remux/copy для Plex. |
| `plex.py` | Plex API, фильмы, сериалы, сезоны, коллекции, постеры, качество, unmatched detection. |
...
| `notified_tasks.json` | Состояние delivery уведомлений, failures, subscribers, Plex polling done и per-chat `plex_poll` delivery для found/timeout; запись уведомлений сохраняет уже выставленные `plex_done`/`plex_poll`. |
...
| `topic_subscriptions.json` | Rutracker/Jackett подписки на новые серии и `type=plex_season` подписки на неполный Plex-сезон; `pending_notification` хранит уже созданный task/update push, если Telegram-доставка упала после успешного добавления задачи. |
...
| `youtube_downloads.json` | Очередь и состояние YouTube-download jobs: metadata preview, выбранное качество, прогресс `yt-dlp`, итоговый путь, ошибка и timestamps. |
| `youtube_plex_refresh_pending.json` | Единственный pending-refresh Plex-библиотеки `YouTube` после удаления роликов, если немедленный refresh не стартовал. |
...
| `download_history.jsonl` | Append-only история событий загрузки по пользователям: добавление, завершение, soft-complete, ошибки, переименование файлов (`files_normalized`) и результат Plex polling; завершение BT-задачи сохраняет `download_destination` и `download_files`, а найденные Plex-сезоны — series-поля для `/seasons`. |
