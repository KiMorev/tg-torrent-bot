...
- `/seasons` builds candidates from Plex shows plus download history, not from DS
  files directly.
- Without useful history or known totals, Plex-only season 5 with 4 episodes is
  not enough for `/seasons` to show a candidate.
- With history using canonical `Clarkson's Farm`, `/seasons` can show a
...
- `/seasons` uses TMDB via Plex external ids to get exact season episode
...
- `/seasons` history matching and active DS-task matching
...
- TMDB season totals are used as `known_totals_by_show` for `/seasons`.
