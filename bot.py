    ...
    if not candidates:
        if scope == "mine" and all_count:
            return (
                ...
                "Как это работает: бот смотрит сезоны Plex, которые выглядят неполными, "
                "и сверяет найденные кандидаты с актуальными раздачами.\n\n"
                "В общей медиатеке есть варианты. Можно переключиться на «Всё»."
            )
        return (
            "📺 <b>Докачать сезон</b>\n\n"
            "Пока не нашёл сезоны, которые можно уверенно продолжить.\n\n"
            "Как это работает: бот смотрит сезоны Plex, которые выглядят неполными, "
            "и сверяет найденные кандидаты с актуальными раздачами. Если появятся новые серии, "
            "здесь можно будет докачать сезон или подписаться на продолжение."
        )
